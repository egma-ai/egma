...
Use `npx skills update integrate-egma` to update the project/global version of this skill installed on the developer's machine.

# Getting started
1. If the `egma` command is unavailable, install it with `npm install --global egma-cli`. This is install a global `egma` cli. Use --help command in the cli to see all the available options.
