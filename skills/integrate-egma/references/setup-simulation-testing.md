...
1. Setup the egma folder and sync state
  ...
  - Unless explicitly asked to use a self hosted version of the egma platform - default to egma cloud platform hosted at https://app.egma.ai/
...
3. Author tests
  ...
  - For pipecat, run the first suite against the bot on this machine through `egma agent dev`, as the pipecat guide says, before any deploy. Then hand over (or run, when allowed) the secrets-and-deploy step.
...
4. Run the tests
- Once there is a suite you wanna run use `egma run --help` to look at how to run a suite against a particular agent and one of its connections. If the agent has multiple connections, default to using the voice connection for the run. Once the run has started, immediately give the developer the link to UI so they can see the run progressing visually. However, your job is only complete once the suite has finished running!
