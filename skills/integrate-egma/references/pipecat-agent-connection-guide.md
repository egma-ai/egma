...
5. The bot needs `EGMA_URL` and `EGMA_API_KEY` wherever it runs. Get the agent's monitoring key with `egma agent monitoring setup --agent "$EGMA_AGENT_ID" --platform pipecat`; it prints both values and the key is shown once. The same key works for `simulation()` and `monitor()`, and a production trace is filed under the egma agent only with this key (with an ordinary project key from `egma project api-key create`, simulations work but production traces show no agent). Use `EGMA_URL=https://app.egma.ai` unless the developer uses a self-hosted egma (then its public URL). For this machine, put both in the bot's gitignored `.env`. Never commit the key and never print it in your messages.
