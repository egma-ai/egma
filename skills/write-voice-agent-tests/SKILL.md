...
D. Mock tools
  ...
  - Never mock a Pipecat Flows function (a function whose handler comes from `pipecat.flows`). Egma does not mock Flows functions, and a simulation whose test mocks one fails when the flow first offers that function. Unmocked Flows functions run for real and are recorded, so write behaviors about them instead.
