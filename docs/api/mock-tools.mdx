...
<ResponseField name="delayMs" type="integer">
...
```bash
curl -X POST https://your-egma-host/v1/mock-tools \
...
`PATCH /v1/mock-tools/:mockToolId`
...
```bash
curl -X PATCH "https://your-egma-host/v1/mock-tools/mck_01abc" \
  -H "Authorization: Bearer <api-key>" \
  -H "Content-Type: application/json" \
  -d '{
...
`DELETE /v1/mock-tools/:mockToolId`

Archive a mock tool so it no longer applies to new runs. Egma keeps the record,
and runs already in progress keep answering the world they froze at start.

Delete a mock tool when you want to stop intercepting a tool call in new runs.
Edit it instead when the tool should stay active but needs a different answer.
...
```bash
curl -X DELETE "https://your-egma-host/v1/mock-tools/mck_01abc" \
  -H "Authorization: Bearer <api-key>"
```
...
<ParamField query="pageToken" type="string">
  The `nextPageToken` value from the previous page. Omit to start from the newest mock tool.
</ParamField>

### Response fields

<ResponseField name="mockTools" type="array">
...
```bash
curl "https://your-egma-host/v1/mock-tools?projectId=prj_abc123" \
  -H "Authorization: Bearer <api-key>"
```
