...
The connection kind must have a compatible adapter installed on this deployment. If you attempt a `phone_number` run on a deployment without a carrier configured, Egma returns `phone_setup_required` before writing anything.

### Request body

<ParamField body="testVersionIds" type="string[]" required>
...
<ResponseField name="agentPlatform" type="string | null">
  Where the customer agent runs, such as `retell` or `livekit_agents`. It can be `null` for a platform-independent connection.
</ResponseField>

<ResponseField name="connectionKind" type="string">
  The simulator path, such as `retell_chat_api`, `phone_number`, or `livekit_room`.
</ResponseField>

<ResponseField name="accessVariant" type="string">
  How Egma gets access to that connection kind, such as `phone_number.public_e164`.
</ResponseField>

<ResponseField name="productLabel" type="string">
  Display text derived from the supported combination. It never selects simulator code.
</ResponseField>

<ResponseField name="modality" type="string">
  `chat` or `voice`, as frozen from the selected connection.
</ResponseField>
...
<ResponseField name="testVersions" type="string[]">
...
<ResponseField name="mockTools" type="object">
  ...
  <Expandable title="mockTools fields">
    <ResponseField name="defaults" type="object[]">
      Project-level mock tools frozen for this run. Each entry has `tool`, `answer` or `error`, `delayMs`, and `mockToolId` (the source row).
    </ResponseField>
    <ResponseField name="overrides" type="object">
      Per-test-version overrides, keyed by `tstv_…` version id. Each value is an array of mock tool entries with the same `tool`/`answer`/`error`/`delayMs` shape.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="byGrader" type="object[]">
  Per-grader verdict breakdown once grading is complete. Each entry includes `graderId`, `required`, `verdict`, `score`, and `counts`.
</ResponseField>

<ResponseField name="gradedCount" type="integer">
  Number of simulations that have been graded so far. This settles independently of execution — a run can have all simulations `completed` but still show `gradedCount` less than `expectedSimulationCount`.
</ResponseField>
...
<ResponseField name="simulations" type="object[]">
  ...
  <Expandable title="simulation fields">
    ...
    <ResponseField name="testId" type="string">The test this simulation is for (go here to edit).</ResponseField>
    <ResponseField name="testName" type="string">The test's current display name.</ResponseField>
    <ResponseField name="testVersionId" type="string">The exact version that was executed (immutable).</ResponseField>
    <ResponseField name="personaId" type="string">The persona used for this simulation.</ResponseField>
    <ResponseField name="personaName" type="string">The persona's display name.</ResponseField>
    <ResponseField name="personaVersionId" type="string">The exact persona version used. This pins the persona instructions and its model selections for the simulation.</ResponseField>
    <ResponseField name="status" type="string">Simulation status: `queued`, `claimed`, `running`, `completed`, `failed`, `canceled`, or `skipped`.</ResponseField>
    <ResponseField name="grading" type="string">`not_required`, `waiting`, `pending`, or `graded`.</ResponseField>
    ...
    <ResponseField name="verdicts" type="object[]">Individual assertion judgments. Each entry has `graderId`, `assertion`, `assertionText`, `required`, `verdict`, `score`, `rationale`, `citedTurns`, and `judgedAt`.</ResponseField>
    ...
    <ResponseField name="mockToolCoverage" type="object | null">Which tools were discovered, covered, and uncovered. `null` if the agent was never asked about its tools.</ResponseField>
    <ResponseField name="modality" type="string">The modality of this specific conversation.</ResponseField>
    <ResponseField name="hasRecording" type="boolean">`true` if a recording exists. Use `GET /v1/simulations/:simulationId/recording` to get a playback URL.</ResponseField>
  </Expandable>
</ResponseField>
...
curl -X POST https://your-egma-host/v1/runs \
  ...
  -d '{
    "testVersionIds": ["tstv_01abc", "tstv_01def"],
    "connectionId": "con_01ghi",
    "idempotencyKey": "release-v2.4.0-voice-tests",
...
```bash
curl "https://your-egma-host/v1/runs/run_01abc" \
  -H "Authorization: Bearer <api-key>"
```
...
<ResponseField name="events" type="object[]">
  ...
  <Expandable title="event fields">
    ...
    <ResponseField name="simulationId" type="string">Present on `simulation` events: the simulation that changed.</ResponseField>
    <ResponseField name="testName" type="string">Present on `simulation` events: the test being executed.</ResponseField>
    <ResponseField name="personaName" type="string">Present on `simulation` events: the persona conducting the call.</ResponseField>
...
```bash
curl "https://your-egma-host/v1/runs/run_01abc/events" \
  -H "Authorization: Bearer <api-key>"
```
...
```bash
curl "https://your-egma-host/v1/runs/run_01abc/events?after=42" \
  -H "Authorization: Bearer <api-key>"
```
...
Stop a run. Simulations still in `queued` state are canceled immediately. Simulations already dispatched to a simulator receive a stop signal and land as `canceled` when they acknowledge it.
...
The response is the full run object as it stands after the cancel, so you can read the final counts without a second request. The `completedCount`, `failedCount`, and `canceledCount` fields remain `null` until all three settle together at the finish.
...
```bash
curl -X POST "https://your-egma-host/v1/runs/run_01abc/cancel" \
  -H "Authorization: Bearer <api-key>"
```
...
```bash
curl "https://your-egma-host/v1/simulations/sim_01abc/recording" \
  -H "Authorization: Bearer <api-key>"
```
