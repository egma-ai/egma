...
A project key cannot read organization-wide member lists, provider-key metadata,
usage or billing, or manage the organization, its members or its projects. It
can create, list and revoke keys only within its own project and its creator's
current role. Browser sessions keep the person's organization permissions when
...
<ParamField body="projectId" type="string">
  ...
  no project cannot file spans. When a project key creates another key, this
  field is required and must match the caller's project. Browser sessions and
  organization keys may omit it for organization-wide access.
</ParamField>
...
<Note>
  ...
  hint. Project keys see only their project's keys. Within the credential's
  scope, admins see every key; other roles see only the keys
  they created.
</Note>
...
When you set up an agent to send traces to Egma, always use a project-scoped
key. The project the key is scoped to is where the traces, runs, and grades
appear.
