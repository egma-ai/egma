...
<Note>
  Prefer your Egma instance's API port for telemetry ingest — port **3100** by default. The web UI port (3101) also reaches this endpoint, because the web application forwards `/v1/traces` to the API, so an exporter pointed at the address you open the dashboard on works; the API is simply one hop shorter. Both may differ in self-hosted deployments — check your `EGMA_API_URL` environment variable, or read the address off the empty **Monitoring** page, which shows the one this deployment answers on.
</Note>
