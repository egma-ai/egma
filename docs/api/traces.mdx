...
<Warning>
  Customer OTLP production ingest requires a project-scoped API key. An organization-wide key is rejected, and Egma stores no spans from that request. Organization-wide keys can still use the trace read APIs to query across the organization. Mint an ingest key via `POST /api/keys` with `project_id` set.
</Warning>
