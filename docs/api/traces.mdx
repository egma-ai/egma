...
OTLP/HTTP from an agent runtime that already emits OpenTelemetry spans. LiveKit
...
<Note>
  Prefer your Egma instance's API port for telemetry ingest — port **3100** by default. The web UI port (3101) also reaches this endpoint because the web application forwards `/v1/traces` to the API. The API is one hop shorter. Both addresses may differ in self-hosted deployments, so check `EGMA_API_URL`.
</Note>
