...
All list endpoints use cursor-based pagination. Responses include a top-level `data` array and a `next_cursor` field:

```json
{
  "data": [...],
  "next_cursor": "cursor_01abc..."
}
```
...
```bash
curl "http://localhost:3100/api/runs?cursor=cursor_01abc..." \
  -H "Authorization: Bearer egma_sk_..."
```

<Tip>
  Always check `next_cursor` rather than comparing the length of `data` to a
