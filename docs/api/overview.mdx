...
OpenTelemetry spans. For LiveKit Agents, use the Egma Python SDK's
`monitor_livekit(ctx)` helper to create or safely reuse the tracer provider and
add the exporter. Retell Monitoring uses API polling instead of this endpoint.
