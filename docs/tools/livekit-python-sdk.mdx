...
description: "Connect your LiveKit Python worker to simulation testing and production monitoring."
---
...
| Egma does not answer the initial tool report | `simulation` raises `egma.NotReported`; the session does not start. |
...
The helper can wait up to 45 seconds for Egma's participant during simulation startup. Do not catch `NotReported` and start the session anyway. Check the room connection, Egma's participant, and the SDK version when this error occurs. Missing settings or an incompatible tracing setup raise `ValueError` instead.
