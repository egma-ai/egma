...
Egma supports [Retell](/guides/retell) and [LiveKit](/guides/livekit). Have the provider credentials available in your terminal or deployment secret store. Keep secret values out of prompts and test files.

For LiveKit production monitoring, follow the [Python SDK](/tools/livekit-python-sdk#monitor-production) or [JavaScript SDK](/tools/livekit-javascript-sdk#monitor-production) instructions. Install the monitoring hook in the worker and deploy it before checking for conversations in Egma.
