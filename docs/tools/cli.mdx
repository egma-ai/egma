...
If an organization-wide or older credential can access several projects, `init` lists their IDs. Run `egma init --project` with the ID you want. A login already scoped to a project does not take `--project`.
...
<Tabs>
  ...
  <Tab title="LiveKit">
    ...
    The worker must be running and registered with that name. See [LiveKit](/guides/livekit) for chat connections, token endpoints, and worker setup.
  </Tab>
</Tabs>
...
Use a persona name from `egma persona list`. Match each mock name and response shape to your agent's real tool. This example assumes `check_availability` returns an object with a `slots` array. LiveKit mocks require the [Python](/tools/livekit-python-sdk) or [JavaScript](/tools/livekit-javascript-sdk) hook.
...
| `egma persona list` | List available persona names and IDs. |
