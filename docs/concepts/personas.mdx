...
To use your own provider account, add its key in
[Settings → Provider API Keys](/configuration/provider-api-keys). Model choices
and provider keys are separate settings; both apply to predefined personas.
