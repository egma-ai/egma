...
description: "Learn how Egma's LLM-based graders judge conversations, how to configure params and scope, and the difference between required and diagnostic graders."
---

A **grader** reads a completed simulation and judges it against the expected behaviors in your tests. After each simulation ends, the grader reads it turn by turn and writes one verdict per expected behavior. Each verdict carries a rationale, the turns it cites, and the grader that produced it. The same grading engine works with simulation transcripts and production telemetry.

## The Grader Library

Egma ships a library of predefined graders, accessible at **Graders** in your dashboard, under the project you are working in (`/projects/:projectId/graders`). Each library entry defines a type of judgment — expected behaviors, latency, tool calls, and so on — along with the form of parameters it accepts.
...
Once you press **Use**, the grader is active for your project. You can list your running graders at `GET /v1/graders`.
...
| `productionSampleRate` | What share of live production traffic this grader judges, as a whole percentage 0–100. |
...
| Change | Effect |
|---|---|
| `params` updated | A new `versionId` is minted. The previous version is preserved. |
| `scope`, `required`, `name`, `description`, or `productionSampleRate` updated | Written in place. No new version. Previous verdicts are unaffected. |
...
Every verdict row references the specific grader `versionId` that produced it, so you can always trace back to the exact configuration that judged a given conversation.

## Configuring graders

### Via the dashboard

Open **Graders** in the project you are configuring (`/projects/:projectId/graders`), find the library entry you want, and click **Use**. Fill in the params form, set the scope, and choose whether the grader is required or diagnostic.

### Via the API

Activate a grader with `POST /v1/graders`:

```bash
curl -X POST https://your-egma/v1/graders \
...
Edit an existing grader with `PATCH /v1/graders/:graderId`. Only the fields you include are updated:

```bash
curl -X PATCH https://your-egma/v1/graders/grd_01K… \
  -H "Authorization: Bearer egma_..." \
  -H "Content-Type: application/json" \
  -d '{ "required": false }'
```
...
Deleting a grader is the only way to turn it off. There is no enable/disable flag. From the moment a grader is deleted, it judges nothing in its scope. Use `DELETE /v1/graders/:graderId`.
