...
Grading waits up to 30 seconds after completion for expected agent evidence.
If it does not arrive, grading can proceed with the available evidence.
[Get a simulation](/api/get-simulation) returns `agentPovIncomplete: true`
when that wait has ended and agent evidence is still missing. A `false` value
does not prove that the export is complete or that existing grades used evidence
that arrived later. Check the worker or provider export, then
[regrade](/guides/configure-graders#regrade-a-simulation) after late evidence
arrives.
...
the start of the agent's reply. For voice, it uses speech boundaries when
available. `first_response_latency` measures the time from the start of the
conversation to the agent's first reply. Both use milliseconds and prefer the
agent's series when it is available. The
