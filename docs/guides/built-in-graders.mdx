...
It reads the conversation's transcript and tool calls. When agent evidence is
available, it uses the agent's account of those turns and calls. See
...
For voice, response time starts when the caller stops speaking and ends when
the agent starts its spoken reply. Egma uses the recorded speech boundaries
when available. The agent's measurement includes the wait while its speech
detector decides that the caller has finished. For chat, it measures the wait
...
Response latency prefers the agent's response latency series
when available and uses Egma's caller timing otherwise. When both exist, they
remain separate series; Egma does not combine their samples. The grader uses
the same primary series shown in the simulation's metrics.
