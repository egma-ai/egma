...
description: "Start a run, wait for execution and grading, and apply your own release policy."
---
...
This recipe uses one explicit policy: **every simulation must complete, and
...
Set up a [supported agent connection](/guides/quickstart), create a suite, and
...
| `EGMA_CI_RUN_KEY` | A unique value for this CI run attempt, reused if its create request is retried. |
...
const idempotencyKey = required("EGMA_CI_RUN_KEY");
...
async function request(path, { method = "GET", body } = {}) {
  // Retrying run creation is safe because its idempotency key stays fixed.
...
function checkSimulation(simulation) {
  ...
  if (simulation.agentPovIncomplete !== false) {
...
try {
  ...
  run = await request("/v1/runs", {
    method: "POST",
    body: {
      suiteId, agentId, connectionId, idempotencyKey,
      name: `CI ${idempotencyKey}`,
  ...
  if (!run && creationRequested) {
    console.error("If creation lost its response, check Runs before retrying. Reuse the same run key and inputs.");
  }
}
```
...
The runner also requires `agentPovIncomplete` to be `false`. This rejects the
API's missing-agent-evidence flag; it does not prove a complete export or that
the current grades used late evidence. If evidence arrives after grading,
...
Use a new `EGMA_CI_RUN_KEY` for a new CI attempt. If a create request loses its
response, retry that request with its original key and identical body. Do not
reuse the key for changed tests or different inputs.
