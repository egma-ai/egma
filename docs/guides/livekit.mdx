...
description: "Run voice and text simulations against your LiveKit agent worker."
---
...
initialized Egma project. Complete [CLI sign-in](/guides/quickstart#1-install-and-sign-in)
first.
...
give Egma. An unnamed worker does not satisfy a named dispatch.
...
If your tests need mock tools, install the SDK for your language and add
`await mockable(agent, ctx, session)` after you create the agent and before
`session.start`. The SDK leaves tools unchanged in production rooms.

<CardGroup cols={2}>
  <Card title="LiveKit Python SDK" icon="python" href="/tools/livekit-python-sdk">
    Install the package and add mock tools to your Python worker.
  </Card>
  <Card title="LiveKit JavaScript SDK" icon="js" href="/tools/livekit-javascript-sdk">
    Install the package and add mock tools to your JavaScript or TypeScript worker.
  </Card>
</CardGroup>
...
[quickstart](/guides/quickstart#3-create-a-suite-and-test) to write a test and
...
Use the returned connection ID to run the [quickstart test](/guides/quickstart#4-start-the-run).
...
To capture real conversations, follow [Set up monitoring](/guides/set-up-monitoring#livekit).
