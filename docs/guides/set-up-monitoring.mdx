...
description: "Capture real conversations from Retell or LiveKit and inspect them in Egma."
---

Retell monitoring imports completed calls with the Retell API. LiveKit
monitoring sends conversation evidence from your worker through the Egma SDK.
...
Complete [CLI sign-in](/guides/quickstart#1-install-and-sign-in) and check
...
<Tabs>
  <Tab title="Python">
    ...
    This requires Python 3.11 or newer and `livekit-agents>=1.6.7,<1.7`.
  </Tab>
...
also appear as production conversations. It works independently of `mockable`;
adding mock tools does not enable monitoring.
...
trace confirms the export path. If no conversation appears, check the worker's
OpenTelemetry logs, the project key, and network access to `EGMA_URL`.

To stop this worker from sending new evidence, remove its `monitor_livekit`
or `monitorLiveKit` call and restart the worker. Existing conversations stay
in Egma.
