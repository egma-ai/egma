...
sidebarTitle: "Retell"
description: "Add text and web-call testing to a Retell agent, then enable production monitoring."
---

You need a Retell agent, a Retell API key that can access it, and an initialized
Egma project in your repository. Complete [CLI sign-in](/guides/quickstart#1-install-and-sign-in)
first.
...
[quickstart run command](/guides/quickstart#4-start-the-run).
...
MCP tools are also outside that mock path.
...
Retell text mode has no stored call record for Egma to import. Its transcript
comes from the simulated conversation itself.
