...
The wizard opens on the terminal's alternate screen. It tells you what it is about to do and waits for one keystroke before it starts. It authorizes the CLI first. Only after sign-in succeeds does it look for installed copies of Claude Code, Codex, Cursor, or OpenCode and ask which one to use. If you are running in CI or want plain output without the interactive UI, pass `--headless`:
...
The CLI asks for your Retell API key (or LiveKit credentials) and then asks one question: **how should Egma test this agent?** Pick as many answers as you want — each one is a connection on the same Egma agent, and one test suite runs over all of them:
...
- **Phone call** — Egma dials the real number, so a run has true telephone latency and reaches your real tools.

<Note>
  Retell is the agent platform, not the connection type. Each lane you pick is one connection: `retell_text_mode` for text, `retell_web_call` for a web call, `phone_number` for the phone. A phone connection can also reach an agent on another or unknown platform.
</Note>
...
```bash
EGMA_RETELL_API_KEY=key_… egma connect --lanes text
# or
cat retell-key.txt | egma connect --lanes text,phone --phone-number +14155550111
```
...
egma/
  ...
  tests/
    receptionist-core/  one direct directory per test suite
      suite.yaml
      *.md
    release-checks/     another suite in the same project
      suite.yaml
      *.md
```
...
project:
  id: prj_01JXMK8P2QN4R6S9T1UV3WX5Y
  name: Voice agents
agents:
  - id: agt_01JXMK8P2QN4R6S9T1UV3WX5Y
    name: Front desk
    connections:
...
<Note>
  ...
  approval. Egma writes the SDK lines automatically only when Git ignores the
  `.env` file. The safe writer refuses links and non-regular files, writes
  atomically, restricts the file to its owner, and prints the lines either way.
</Note>
...
The first generated suite contains exactly four tests. Before starting the run,
the wizard shows the full list and pauses:

```
4 tests generated · suite "Receptionist core"
...
Run the wizard again to add another target or another suite. It keeps the
agents, connections, and suite directories already recorded in `egma/`.
...
A suite is project-owned, not agent-owned. A run selects the suite, agent, and
connection together. If the config has more than one runnable agent or the
