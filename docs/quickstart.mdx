...
<Note>
  Retell is the agent platform, not the connection kind. The saved connection is `retell_chat_api` for a chat agent or `phone_number` for a voice agent. A phone connection can also reach an agent on another or unknown platform.
</Note>
