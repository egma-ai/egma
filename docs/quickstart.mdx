...
The CLI asks for your Retell API key (or LiveKit credentials), reads the selected Retell agent's modality, and asks you to confirm the matching connection:

- A Retell **chat agent** gets a Retell connection with the **Chat** modality. Egma exchanges messages with it directly.
- A Retell **voice agent** gets a Phone connection with the **Voice** modality. Egma dials one of the numbers routed to it.

<Note>
  Egma does not offer both choices for one Retell agent. A voice agent requires `--reach phone`; a chat agent requires `--reach text`. A mismatch stops before Egma writes anything.
</Note>
