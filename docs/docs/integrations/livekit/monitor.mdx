...
<Tabs>
  ...
  <Tab title="JavaScript">
    ...
    Create the session, then pass it to the hook before `ctx.connect` or
    `session.start`:

    ```typescript
    const session = new voice.AgentSession({
      // Your existing session options.
    });
    monitor(ctx, { session });
    await ctx.connect();
    ...
    Passing the session includes greetings and other `session.say()` messages
