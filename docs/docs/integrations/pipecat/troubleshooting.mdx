...
| `Egma could not reach your start URL` | Check that your starter is running and that its URL resolves and answers over HTTPS. For a `trycloudflare.com` address, run `egma agent dev` on the machine that runs your bot. |
...
| `the test mocks "…", and this is a Pipecat Flows function` | Egma cannot mock a Pipecat Flows function yet. The simulation fails when the flow first offers that function. Remove it from the test's mock tools. Flows functions that are not mocked run normally and are recorded. |
...
| `your bot has RTVI turned off` | Chat simulations need RTVI. Remove `enable_rtvi=False` from your `PipelineWorker`. Voice simulations still work without it. |
...
- If simulations stop reaching your computer after it sleeps or changes network, restart `egma agent dev`.
