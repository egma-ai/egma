...
Copy the key when the CLI prints it; it is shown only once. If the bot will also [send production conversations](/docs/integrations/pipecat/monitor#get-the-agents-monitoring-key), use the agent's monitoring key instead; it works for simulations too. Your bot needs these two settings:
