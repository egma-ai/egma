...
[Get a simulation](/api-reference/get-simulation) returns `agentPovComplete` when the final agent record is present. `agentPovIncomplete` is true when the record is explicitly degraded, or still missing after the wait. Existing grades do not change when late evidence arrives; [regrade](/docs/platform/graders/configure-graders#regrade-a-simulation) to use it.
...
pass/fail result to a run. See [Scores and thresholds](/docs/platform/graders/scores-and-thresholds).
...
[Response latency grader](/docs/platform/graders/built-in-graders#response-latency) uses the
...
[Automate tests in CI](/docs/platform/runs/automate-tests-in-ci).
