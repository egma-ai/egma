...
When the run is created, Egma saves the test and persona versions, persona
settings, and grader selection, versions, settings, and thresholds. Queued
simulations and retries use that saved selection. A new persona clone or a
grading policy change applies only to future runs.
