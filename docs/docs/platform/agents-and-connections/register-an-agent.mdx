...
<Tabs>
  <Tab title="UI">
    ...
    This short walkthrough shows how to start LiveKit voice setup, up to the connection details. See the [LiveKit guide](/docs/integrations/livekit) for the required worker and SDK setup.

    <video controls autoPlay loop muted playsInline preload="metadata" aria-label="Start registering a LiveKit agent" poster="/docs/assets/videos/register-livekit-agent-poster.jpg">
      <source src="/docs/assets/videos/register-livekit-agent.mp4" type="video/mp4" />
    </video>
  </Tab>
  <Tab title="CLI">
    First, [install the CLI, sign in, and initialize your project](/skills-cli-sdks/cli/install#sign-in-and-initialize).
