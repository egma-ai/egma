...
The grader receives the transcript, tool names and arguments, observed metrics,
and any expected behaviors from the test. Tool return payloads are not sent to
the LLM. Use rules you can check with this evidence.
The transcript and tool calls prefer the agent's evidence when available.
See [Check which evidence arrived](/docs/platform/runs/review-a-run#check-which-evidence-arrived).
...
3. Check cases where the relevant action does not occur, the caller declines, or
   evidence is missing.
4. Set the [scope and pass threshold](/docs/platform/graders/configure-graders) for future
   conversations once the grader gives useful results.

## Read the score

A check of one instruction returns `1` when met. It returns `0` when not met
or when the model cannot determine whether it was met. A prompt that checks each
supplied expected behavior can return a fractional score: the number of met
behaviors divided by the total, including undetermined behaviors. Egma computes
this score from the decisions; the model does not choose an arbitrary numeric score.

An undetermined decision earns no credit toward the pass threshold. Its original
decision, rationale, and citations remain in the grade details. A failed model
request or an incomplete or invalid response produces a grading error with no
score. Read the rationale and citations before changing your threshold. See
