...
It reads the conversation's transcript, tool names and arguments, and observed
metrics. Tool return payloads are not sent to the LLM.
When agent evidence is available, it uses the agent's account of the turns and
calls. See
[Check which evidence arrived](/docs/platform/runs/review-a-run#check-which-evidence-arrived)
...
Use [custom graders](/docs/platform/graders/custom-graders) for a rule that should apply across
