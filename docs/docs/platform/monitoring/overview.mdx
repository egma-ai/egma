...
You do not need tests or simulation runs to monitor an agent. LiveKit and
Pipecat send production traces through the Egma SDK. Retell imports completed
voice calls through its API.
...
LiveKit, or Pipecat. Complete a real conversation and verify that its evidence arrives
