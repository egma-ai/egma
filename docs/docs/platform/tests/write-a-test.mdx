...
For Pipecat, replace `job_dispatch_metadata` with `pipecat_body_params`. Egma adds this JSON object to the start request's `body`, and your bot reads it at `runner_args.body`. The key `egma` is reserved, and the object can be at most 512 KiB. See [Pass data to a test](/docs/integrations/pipecat/text-and-env#pass-data-to-a-test).
...
<Note>
  ...
  helper before the agent session starts, including tests without mocks.
  Every Pipecat simulation requires it before the runner starts the worker. See
