...
<Tabs>
  <Tab title="UI">
    1. Open **Tests**, select your suite, and find the test.
    2. Select its **Mock tools** cell to open the JSON editor.
    3. Enter an array of tool mocks. Use the exact tool name and its normal response shape:
    ...
    4. Select **Save**. If you are creating a new test, also select **Save test** to save the full test.
  ...
  <Tab title="CLI">
    ...
    Run `egma push` to save the changed test to Egma.
  </Tab>
</Tabs>
...
Use `error` with a string to make the tool fail. In the UI's Mock tools editor,
...
| LiveKit | Call `await simulation(agent, ctx, session)` from the [Python SDK](/skills-cli-sdks/sdks/livekit-python) or [JavaScript SDK](/skills-cli-sdks/sdks/livekit-javascript) before `session.start`. |
| Pipecat | Call `await simulation(worker, runner_args)` from the [Pipecat Python SDK](/skills-cli-sdks/sdks/pipecat-python) before `runner.add_workers(worker)`. |

Retell phone calls do not apply these mock answers. Use text mode or a web call
when your Retell test depends on mocks.

For LiveKit and Pipecat, the `simulation` helper is required even when a test
has no mocks. It sends the agent's spans, reports its tools, and applies the
running test's mock answers. Set `EGMA_URL` and `EGMA_API_KEY` in the agent's
environment. If the setup exchange fails, the helper raises `NotReported` and
the agent does not start. The `monitor` helper is for production conversations;
it does not enable simulations.

Egma cannot mock a Pipecat Flows function yet. A Pipecat simulation whose test
mocks one fails when the flow first offers that function, with a message that
names it.
...
If you edited a local test file, run `egma push`. Then [start a run](/docs/platform/runs/start-and-follow-a-run). Open the
...
LiveKit or Pipecat SDK setup. If the agent reacts incorrectly to the answer, compare
