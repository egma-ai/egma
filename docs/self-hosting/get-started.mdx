---
title: "Get started"
description: "Run Egma on your own machine with Docker Compose."
---
...
You supply model access for your instance. Egma stores agent provider credentials separately when you connect a Retell agent or LiveKit project.
...
Approve the login in your browser. Then follow the [quickstart](/guides/quickstart) to connect Retell or LiveKit, write a test, and run it.

For LiveKit monitoring, set the worker's `EGMA_URL` to an address it can reach. A worker deployed elsewhere cannot send traces to this machine through `localhost`.
