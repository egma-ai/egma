...
| Your LiveKit worker's environment | `EGMA_URL` and the project-scoped `EGMA_API_KEY` used to send production traces. |
...
From another machine, verify that you can sign in at the public instance URL, run a test, and play its recording. Retell must be able to reach the instance URL for mock-tool callbacks. LiveKit workers must be able to reach it to send monitoring traces.
