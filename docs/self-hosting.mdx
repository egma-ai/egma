...
The platform is ready. Chat and text simulations can run without a carrier.
...
Either origin reaches the same door: the ingest endpoint lives on the API (`3100`), and the web application (`3101`) forwards `/v1/traces` to it. The API is one hop shorter. Leave `/v1/traces` off the end — the exporter appends it.

Mint the exporter's API key against a specific project. A key that names the whole organization files its spans under no project, and none of them appear in the dashboard.
