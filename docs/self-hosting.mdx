...
Open `.env` in an editor. Most values have working defaults and you can leave them alone. **Twelve variables have no default and the platform refuses to start without them.** They are marked `REQUIRED` in `.env.example`. The sections below explain each one and how to generate it.

## Step 2 — Set the required secrets

Nine of the twelve required values are secrets you generate yourself. The remaining three — the LiveKit API key and secret, and `EGMA_BASE_URL` — are set automatically when you run `self-host up`.
...
### The ingestion bucket, and one limit to know about

The same MinIO service holds a second bucket for production evidence. Egma
answers a customer's trace request successfully only after the evidence is in
that bucket, so this is the point at which Egma promises the evidence is safe.
It is a spool rather than an archive: Egma writes the evidence to ClickHouse
soon afterwards and then deletes the object.

```bash
# The ingestion pair — the API writes accepted evidence with it
EGMA_S3_INGEST_ACCESS_KEY_ID=your-chosen-ingest-key-id
EGMA_S3_INGEST_SECRET_ACCESS_KEY=your-chosen-ingest-secret
```

A separate bucket and a separate credential from recordings, on purpose: this
pair can only write, list, read and delete inside the ingestion bucket's
`pending/` prefix, so it can never touch a recording.

<Warning>
  **The bundled MinIO runs on one node, so its durability is your host's.** The
  volume survives a container replacement and a restart. It does not survive the
  loss of that host or that disk. If your production evidence must survive
  either, point `EGMA_INGEST_ENDPOINT` at a store that is remote from the
  machine Egma runs on — AWS S3 or any S3-compatible service with its own
  durability promise.

  `docker compose down -v` deletes the volumes, which deletes the ingestion
  bucket and the local write-ahead log with them. Any evidence Egma had accepted
  and not yet written to ClickHouse goes with them.
</Warning>

### Summary of all twelve required variables
...
| `EGMA_S3_INGEST_ACCESS_KEY_ID` | Ingestion bucket credential (write, `pending/` only) | Choose a strong random string |
| `EGMA_S3_INGEST_SECRET_ACCESS_KEY` | Ingestion bucket secret (min 8 chars) | Choose a strong random string |
...
3. Prints the URL when the health check succeeds
...
The platform is ready. Open `http://localhost:3101`, sign up, and claim the instance. You become
...
provider, and voice. LLM grader versions choose the model they use. The
deployment keys above supply credentials only. They are not copied into
Postgres.
...
Open **Monitoring → Start monitoring** inside the project. Which steps you take
depends on the agent platform.

- **Retell:** enter one Retell API key, choose which Retell agent your Egma
  agent is, and start. The API service seals the key on that agent, imports the
  previous 30 days, and then polls for completed conversations.
- **LiveKit:** install the Egma Python SDK in the agent process. Set
...
If another agent process already creates and registers an OpenTelemetry
...
organization-scoped key is rejected before Egma decodes or stores the export.
...
The first person to sign up claims the organization and becomes its admin. After that, signup is closed and new users arrive by invitation only — admins send invitations from **Settings → People**. If you are building a multi-tenant deployment, set this to `false`.

## Adding a second user

Open a project in the web application, choose **Settings → People**, enter an email address, select a role, and send. If no SMTP server is configured, the invitation link comes back to you directly in the response — paste it to your colleague however you like (Slack, email, a text message).
...
<Accordion title="Platform refuses to start and names a missing variable">
  Every required variable is named in `.env.example`. Make sure you have filled in all nine secrets listed in the table above. `EGMA_LIVEKIT_API_KEY`, `EGMA_LIVEKIT_API_SECRET`, and `EGMA_BASE_URL` are set automatically by `egma self-host up` — if they are missing, run `egma self-host up` instead of `docker compose up`.
</Accordion>
