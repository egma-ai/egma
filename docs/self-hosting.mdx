...
4. **Phone carrier** — a trunk address, source number, SIP username, and SIP password (optional — press Enter to skip)
...
If you are upgrading from a release that used `egma self-host phone setup`, do not reuse its global SIP credential on every machine. Ask a Twilio administrator to create one credential per developer in the existing credential list. Save each four-value bundle outside the database, run `egma self-host setup --replace-carrier --yes` on each existing local platform, and run one phone simulation. After every platform has moved and passed, the administrator can revoke the old shared credential. The old `.egma-platform/platform.env` file stays because it also holds the LiveKit key and secret.
