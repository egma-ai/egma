...
<AccordionGroup>
  ...
  <Accordion title="egma self-host up — start a local Egma instance">
    ...
    A complete phone route in `.env` replaces the sealed runtime copy. With no
    environment route, an existing stored route is preserved for safe upgrades.
    For Twilio, use the SIP credential-list username and password, never the
