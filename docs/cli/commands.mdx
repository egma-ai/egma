...
<AccordionGroup>
  ...
  <Accordion title="egma self-host setup — configure a running instance">
    ...
    For phone setup, Egma asks for a trunk address, source number, SIP username, and SIP password. A Twilio administrator creates that limited credential in the list already attached to the shared trunk. Setup copies the four values into the platform as one bundle. It never asks for the Twilio Account SID or Auth Token and never reads or changes Twilio.
