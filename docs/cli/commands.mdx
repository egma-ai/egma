...
<AccordionGroup>
  ...
  <Accordion title="egma run — run your test suite">
    ...
    **Exit codes**
    ...
    <Note>
      A grader can return `skipped` when its own modality does not apply. Egma
      does not skip a test because of a connection capability. Every test in
      the suite gets a simulation.
    </Note>
  </Accordion>
