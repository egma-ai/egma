...
Everything under `egma/` is committed. The `config.yaml` file records the non-secret platform URL that owns these resource identifiers, so every command run in this repository finds it automatically.
...
|---|---|---|
| `format` | No | The file format Egma writes. A new hand-authored file can omit it |
| `name` | Yes | A short identifier for the test. Used as the filename and displayed in run output |
| `description` | No | A short explanation shown beside the name |
| `version` | No | The test-content version. `egma pull` and `egma push` write it |
| `identity_revision` | No | The synced revision of the name and description. `egma pull` and `egma push` write it |
| `required_capabilities` | No | Connection features that this test needs, such as `dtmf` |
| `personas` | No | Persona identities used for the simulation. Omit the field to use the default persona |

Name a persona only when the situation calls for a particular kind of caller. Use an identity already written by `egma pull`; do not invent an ID. Most tests work fine with the default.
...
<Steps>
  <Step title="Pull Egma's current versions">
    Run `egma pull` to write Egma's current version of every test into your local files. This updates the sync fields and known persona identities in each file's front matter.
