...
**Retell** is an agent platform with two simulation paths. A Retell chat agent uses the Retell Chat API. A Retell voice agent uses a public phone number. Production Monitoring is separate: enter one Retell API key on the Monitoring setup page and select the voice agents whose completed conversations Egma should poll.
