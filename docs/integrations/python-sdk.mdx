...
1. **Reports the agent's tools** by name and schema. Mock authoring in Egma starts from your real tool names and signatures, not from your memory of them.
2. **Asks Egma which tools this simulation answers for.** Egma replies with exactly the tool names the running test writes under `## Mock tools`, and nothing else.
3. **Installs a courier** in front of each of those names using LiveKit's own `mock_tools`. Couriers go into a side table LiveKit keeps per session; they do not touch your agent, your agent's class, or your tool registry. The model keeps seeing your real tool schemas throughout.
4. **Follows LiveKit handoffs.** Before a selected `Agent` or `AgentTask` starts, the SDK installs its couriers for that agent's tools too.
5. **Leaves every other tool exactly as it was.**

**Egma wraps exactly the tools the running test names. Every other tool runs its real implementation, and Egma is not in that path.** A test that names none runs your agent against your real backend from end to end.

Each call a courier carries lands on the simulation record with its arguments, the answer Egma served, how long it took, and the name of the mock tool that answered it. A call that ran your real implementation is not on the record at all, because Egma never saw it.
...
Call `mockable` once on the initial agent, after the agent object exists and before `session.start`. Its first message is the first thing sent, so an Egma that is not in the room is found before any tool call rather than halfway through a test.
