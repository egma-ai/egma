...
- **The dispatch carries your test's env, and nothing of Egma's.** LiveKit teaches dispatch metadata as the channel for a caller's own context, so where Egma dispatches the job itself — a connection that holds your project key pair — it writes the running test's `job_dispatch_metadata` there, as one compact JSON string, byte for byte, key for key. An agent doing `json.loads(ctx.job.metadata)["tenant"]` in production does the same thing in a simulation and reads the value that scenario meant it to read. A test that names no env dispatches the empty string, and a connection through your own token endpoint gets no dispatch from Egma at all. Egma adds no key of its own, and it leaves the room's metadata empty. Egma reads its own answer from the room's name. See [Agents and connections](/concepts/agents-and-connections).
