...
| Field | Meaning |
|---|---|
| `room_name` | A new room for this simulation. It always starts with `egma-sim-`. Egma never creates it first and never reuses it. |
...
| `room_config.agents[0].metadata` | The agent metadata on the connection, byte for byte. Sent only when the connection has one. Your worker reads it at `ctx.job.metadata`. |

Egma sends nothing else. No persona, no scenario, no participant metadata or
attributes. The room name is the only signal that a simulation is running.
...
In the app, choose LiveKit, voice, and the connection type **Token endpoint**.
The form asks for the LiveKit agent name, the token endpoint, the auth headers,
and optional agent metadata. It does not ask for your LiveKit server URL. Your
endpoint answers with it.

Through the API, `tokenEndpoint` and `agentName` go in the config, `metadata`
is optional, and the auth headers are a credential:
