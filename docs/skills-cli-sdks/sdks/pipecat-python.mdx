...
Chat simulations need no extra code. In a chat simulation, the SDK turns your bot's speech output off after Egma confirms the simulation, so the greeting and every answer stay text only. Chat needs RTVI, which `PipelineWorker` turns on by default, so do not pass `enable_rtvi=False`.
