...
Run the same install command to update the CLI. If you use a coding agent, install the [Egma skills](/skills-cli-sdks/skills/install) too.
...
<Tabs>
  <Tab title="Retell">
    ...
    The first connection saves the Retell agent ID and credential in Egma. Later setup can reuse them. For the other Retell connection choices, see [Retell](/docs/integrations/retell).
  </Tab>
  <Tab title="LiveKit">
    ...
    The worker must be running and registered with that name. Install the [Python SDK](/skills-cli-sdks/sdks/livekit-python) or [JavaScript SDK](/skills-cli-sdks/sdks/livekit-javascript), set the worker's `EGMA_URL` and project-scoped `EGMA_API_KEY`, and call `await simulation(agent, ctx, session)` before its session starts. This is required for every LiveKit simulation, including tests without mocks. See [LiveKit](/docs/integrations/livekit) for connection and worker setup.
  </Tab>
</Tabs>
...
Use a persona name from `egma persona list`. Match each mock name and response shape to your agent's real tool. This example assumes `check_availability` returns an object with a `slots` array. LiveKit simulations require the [Python](/skills-cli-sdks/sdks/livekit-python) or [JavaScript](/skills-cli-sdks/sdks/livekit-javascript) hook.
...
For LiveKit, use `job_dispatch_metadata` instead. The worker reads that value as JSON from `ctx.job.metadata`. Put only the field your platform uses in the test. See [Write a test](/docs/platform/tests/write-a-test) for test design and [Personas](/docs/platform/personas) for persona selection.
...
Open the printed results URL. The command returns after the run starts; its success exit code does not mean the simulations have finished or passed. See [Start and follow a run](/docs/platform/runs/start-and-follow-a-run).
...
For LiveKit, add the monitoring hook to your worker using the [Python SDK](/skills-cli-sdks/sdks/livekit-python#monitor-production) or [JavaScript SDK](/skills-cli-sdks/sdks/livekit-javascript#monitor-production). The CLI's LiveKit monitoring commands return setup guidance and exit with code `1`; they do not change the worker.
...
The CLI uses `EGMA_API_KEY` when it is set, so CI does not need browser login. Use [Automate tests in CI](/docs/platform/runs/automate-tests-in-ci) to wait for completion and make a release decision with the API.
...
The CLI has no JSON output mode or command that waits for a run's grades. Use the [API reference](/api-reference/overview) for scripts that need structured results.
