...
Open the printed results URL. The command returns after the run starts; its success exit code does not mean the simulations have finished or passed. See [Start a run](/docs/platform/runs/start-and-follow-a-run).
...
The CLI uses `EGMA_API_KEY` when it is set, so CI does not need browser login.
