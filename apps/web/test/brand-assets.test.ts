 ...
 *
 * It happened. `main` replaced the PNG brand pair with SVGs and deleted
 * `egma-logo.png`; the product shell — this effort's own file, so no conflict
 * — kept pointing at it, which put a broken mark in the sidebar of every
 * screen at once. This file is the guard that would have said so.
 *
...
describe("what the application asks the browser to fetch", () => {
  ...
  it("finds the brand mark the shell draws, so this test can fail", () => {
    // A guard that matches nothing passes for ever. The shell's own mark is
    // the reference this file exists for, so it is asserted to be one of the
    // paths the scan above collects.
    const shell = readFileSync(path.join(WEB, "ui", "shell.tsx"), "utf8");
    const marks = [...shell.matchAll(ASSET)]
      .map(([, referenced]) => referenced as string)
      .filter((asset) => SERVED_FROM_PUBLIC.test(asset));

    expect(marks.length, "the shell names no asset for this test to hold").
      toBeGreaterThan(0);
