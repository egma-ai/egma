...
describe("getting to the Library screen", () => {
  /**
   * **Nothing in the product navigation points here, and that is wave one's
   * decision rather than an oversight.**
   *
   * This screen is organization-wide: its address carries no project, and the
   * shell reads the project out of the address and falls back to whichever one
   * is first in the viewer's list. So a person with three projects who pressed
   * **Use** here would put a running copy on a project they were not looking
   * at, with nothing on screen saying which. A screen that quietly acts on the
   * wrong project is worse than no screen.
   *
   * It stays in the tree as the working reference for this copy and this Use
   * form. Wave two rebuilds it project-scoped, and the day it does, a
   * `graders` item goes back into `PRIMARY_NAVIGATION` and this test says so
   * again.
   */
  it("is deliberately not in the product navigation yet", () => {
