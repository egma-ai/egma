...
| `EGMA_SIMULATOR_CAPACITY` | `2` | Most simulations conducted at once. A voice simulation costs a channel on the deployment's carrier trunk, so this is also what keeps a big run inside what the carrier allows: raise it only as far as the trunk really goes. |
