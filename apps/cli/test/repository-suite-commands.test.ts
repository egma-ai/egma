...
function testBody(input: {
  ...
  return {
    ...
    description: "",
    scenario: input.scenario ?? "The caller asks for Tuesday.",
    expectedBehaviors: input.expectedBehaviors ?? ["The agent books Tuesday."],
    personas: [],
...
describe("complete repository suite commands", () => {
  ...
  it("refuses duplicate returned file receipts without writing either local pin", async () => {
    const release = await suite("release", SUITE_ID, "Release");
    const files = ["books-a-visit.md", "cancels-a-visit.md"] as const;
    for (const [index, file] of files.entries()) {
      await writeFile(
        path.join(release, file),
        serializeTestFile(
          aTestFile({
            name: index === 0 ? "Books a visit" : "Cancels a visit",
            scenario: "The caller asks about a visit.",
            expectedBehaviors: blocking("The agent handles the visit."),
          }),
        ),
    ...
    const code = await runPushCommand({
      ...
      fetchImpl: async (_input, init) => {
        ...
        const duplicate = body.tests[0]!.clientRef;
        return new JsonResponse(
          JSON.stringify({
            tests: [
              {
                clientRef: duplicate,
                test: testBody({ id: TEST_ID, versionId: VERSION_ID }),
              },
              {
                clientRef: duplicate,
                test: testBody({
    ...
    expect(failed.join("\n")).toContain("duplicate");
    expect(failed).toContain("Run egma pull.");
    expect(failed.join("\n")).not.toContain("Nothing was uploaded");
    for (const file of files) {
      expect(await readFile(path.join(release, file), "utf8")).not.toContain("version:");
    }
  });

  it("refuses same-Suite Test receipts mapped to the other local files", async () => {
    const release = await suite("release", SUITE_ID, "Release");
    const files = [
      ["books-a-visit.md", "Books a visit", "The caller asks to book."],
      ["cancels-a-visit.md", "Cancels a visit", "The caller asks to cancel."],
    ] as const;
    for (const [file, name, scenario] of files) {
      await writeFile(
        path.join(release, file),
        serializeTestFile(
          aTestFile({
            name,
            scenario,
            expectedBehaviors: blocking("The agent handles the request."),
          }),
        ),
      );
    }
      ...
      fetchImpl: async (_input, init) => {
        ...
        return new JsonResponse(
          JSON.stringify({
            tests: [
              {
                clientRef: body.tests[0]!.clientRef,
                test: testBody({
                  id: SECOND_TEST_ID,
                  versionId: SECOND_VERSION_ID,
                  name: "Cancels a visit",
                  scenario: "The caller asks to cancel.",
                  expectedBehaviors: ["The agent handles the request."],
                }),
              },
              {
                clientRef: body.tests[1]!.clientRef,
                test: testBody({
                  name: "Books a visit",
                  scenario: "The caller asks to book.",
                  expectedBehaviors: ["The agent handles the request."],
                }),
              },
            ],
    ...
    expect(output.join("\n")).toContain(TEST_ID);
    expect(output.join("\n")).toContain(SECOND_TEST_ID);
    expect(failed.join("\n")).toContain("content or ownership");
    expect(failed).toContain("Run egma pull.");
    for (const [file] of files) {
      expect(await readFile(path.join(release, file), "utf8")).not.toContain("version:");
    }
  ...
  it.each([
    ["a different Project", { projectId: "prj_other" }],
    ["a different Suite", { suiteId: EMPTY_SUITE_ID }],
    ["different authored content", { scenario: "A different situation." }],
    ["different Mock Tools", { mockTools: [{ tool: "calendar", answer: null }] }],
    ["different environment", { env: { retell_dynamic_variables: { caller: "Ada" } } }],
    ["a blank revision", { revision: "" }],
    ["a non-positive version", { version: 0 }],
  ] as const)(
    "refuses a repository receipt with %s before writing a local pin",
    async (_case, returned) => {
      const release = await suite("release", SUITE_ID, "Release");
      ...
      expect(output.join("\n")).toContain(VERSION_ID);
      expect(failed).toContain("Run egma pull.");
      expect(await readFile(file, "utf8")).not.toContain("version:");
    },
  );

  it("keeps durable IDs when a repository receipt omits its clientRef", async () => {
    const release = await suite("release", SUITE_ID, "Release");
    const file = path.join(release, "books-a-visit.md");
    await writeFile(
      file,
    ...
    const code = await runPushCommand({
      ...
      fail: (line) => failed.push(line),
      fetchImpl: async () =>
        new JsonResponse(JSON.stringify({ tests: [{ test: testBody() }] })),
    });

    expect(code).toBe(1);
    expect(output.join("\n")).toContain(TEST_ID);
    expect(output.join("\n")).toContain(VERSION_ID);
    expect(output.join("\n")).toContain("unknown local file");
    expect(failed).toContain("Run egma pull.");
    expect(await readFile(file, "utf8")).not.toContain("version:");
  });

  it("fails closed when an applied repository has no Test receipt collection", async () => {
    const release = await suite("release", SUITE_ID, "Release");
    const file = path.join(release, "books-a-visit.md");
    await writeFile(
      file,
    ...
    const code = await runPushCommand({
      ...
      out: () => {},
      fail: (line) => failed.push(line),
      fetchImpl: async () => new JsonResponse(JSON.stringify({ tests: {} })),
    });

    expect(code).toBe(1);
    expect(failed.join("\n")).toContain("without a Test receipt collection");
    expect(failed).toContain("Run egma pull.");
    expect(await readFile(file, "utf8")).not.toContain("version:");
  });
  ...
  it("keeps local suite and test identities when they were deleted remotely", async () => {
    ...
    try {
      ...
      expect(
        (await fetch(
          `${platform.url}/v1/tests/${deletedTest.id}` +
            `?expectedVersionId=${deletedTest.versionId}` +
            `&expectedRevision=${deletedTest.revision}`,
          {
            method: "DELETE",
            headers,
          },
