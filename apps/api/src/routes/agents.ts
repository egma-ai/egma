 ...
 * no business editing them; what egma owns is the name and
...
function describedAgent(one: Agent): Record<string, unknown> {
  return {
    ...
    name: one.name,
    // How this agent's production traffic reaches egma: the platform binding,
    // the hint of the sealed monitoring key, and the one declared switch. The
    // key itself is never in a read.
    agentPlatform: one.agentPlatform,
    platformAgentId: one.platformAgentId,
    monitoringKeyHint: one.monitoringApiKeyHint,
...
export async function agentRoutes(
  ...
  /**
   * The Egma-owned half of an agent.
   *
