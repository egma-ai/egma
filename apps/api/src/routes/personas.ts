...
type Query = {
  ...
  readonly mode?: "separate" | "live"; readonly liveProvider?: string; readonly liveModel?: string;
...
export function requestSignal(
  request: { raw: { once(event: "aborted", listener: () => void): unknown } },
  reply: { raw: { once(event: "close", listener: () => void): unknown; writableEnded: boolean } },
): AbortSignal {
...
async function settingsRefusal(
  ...
  if (models.mode === "live") {
    const capabilities = resolvePersonaCapabilities({ mode: "live", liveProvider: models.live.provider, liveModel: models.live.model, language: controls.language, voiceId: models.live.voiceId });
    return personaCapabilityRefusal(capabilities, { ...controls, voiceId: models.live.voiceId, voiceField: "models.live.voiceId" });
  }
...
export async function personaRoutes(
  ...
  registerPlatformOperation(app, personaOperations.getPersonaCapabilities, async (request, reply) => {
    ...
    const selection = {
      ...
      liveProvider: text(query.liveProvider), liveModel: text(query.liveModel),
    ...
    if (selection.ttsProvider === "cartesia") {
      ...
      if (cached !== undefined && cached.expires > Date.now()) voices = cached.voices;
        ...
        try {
          const discovered = await discoverCartesiaVoices(credential.key, fetch, requestSignal(request, reply));
          voices = customer === undefined ? discovered.filter((voice) => voice.publiclyAccessible === true) : discovered;
        } catch {
