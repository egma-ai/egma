...
export async function persistProviderUsage(
  ...
  const spans = [providerUsageSpan(record)];
  try {
    ...
    console.error(
      "provider usage could not reach the durable ingestion bucket; any staged WAL record remains recoverable",
      cause,
    );
  }
  try {
    ...
    console.error(
      "provider usage was not appended; recover the same attempt from durable ingestion, or absorb unprovable cost",
      cause,
    );
  }
}
