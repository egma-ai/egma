...
export const simulation = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.endedAt} is not null or ${table.recordingReference} is null`,
    ),
    ...
    // A chat has no audio, so the row refuses to hold a recording for one.
    check(
      "simulation_audio_facts_are_voice_facts",
      sql`${table.modality} = 'voice' or ${table.recordingReference} is null`,
    ),
