...
/** Historical authored default. Current runtime gain is fixed outside storage. */
...
type HistoricalPersonaControls = PersonaControls & {
...
function modelParameterValues(models: PersonaModels, checkCurrentCatalog = true, includeHistoricalSpeed = false): PersonaParameterValues {
  ...
  return {
    ...
    ...(includeHistoricalSpeed ? { tts_speed: checked.tts.speed } : {}),
  };
}
...
export function legacyPersonaParameterContract(models: PersonaModels = RECOMMENDED_PERSONA_MODELS): readonly GraderParameter[] {
  const defaults = modelParameterValues(models, false, true);
...
export function validPersonaControls(value: unknown, mode?: PersonaModels["mode"]): PersonaControls {
  if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("persona controls must be an object");
  const held = value as Record<string, unknown>;
  const supported = new Set(["models", "language", "executionPolicyVersion", "backgroundSoundId", "interruptionLevel"]);
  const unsupported = Object.keys(held).filter((key) => !supported.has(key));
  if (unsupported.length > 0) throw new TypeError(`persona controls have unsupported fields ${unsupported.sort().join(", ")}`);
  ...
  if (mode === "live" && held.interruptionLevel !== undefined) throw new TypeError("live personas do not support interruptions");
  if (mode === "separate" && !PERSONA_INTERRUPTION_LEVELS.includes(held.interruptionLevel as PersonaInterruptionLevel)) throw new TypeError("cascaded personas need a supported interruption level");
  if (mode === undefined && held.interruptionLevel !== undefined && !PERSONA_INTERRUPTION_LEVELS.includes(held.interruptionLevel as PersonaInterruptionLevel)) throw new TypeError("persona interruption level is not supported");
  return {
    language,
    executionPolicyVersion: held.executionPolicyVersion,
    backgroundSoundId: held.backgroundSoundId as BackgroundSoundId,
    ...(held.interruptionLevel === undefined ? {} : { interruptionLevel: held.interruptionLevel as PersonaInterruptionLevel }),
  };
}

function validateHistoricalControls(values: PersonaParameterValues, background = false): void {
  if (typeof values.language !== "string" || values.language.trim() === "") throw new TypeError("persona language must be nonempty text");
  if (!PERSONA_EMOTIONS.includes(values.emotion as PersonaEmotion)) throw new TypeError("persona emotion is not supported");
  if (typeof values.accent !== "string" || values.accent.trim() === "") throw new TypeError("persona accent must be nonempty text");
  if (typeof values.speech_volume !== "number" || values.speech_volume < SPEECH_VOLUME_RANGE.quietest || values.speech_volume > SPEECH_VOLUME_RANGE.loudest) throw new TypeError("persona speech volume is invalid");
  if (background && !BACKGROUND_SOUND_IDS.includes(values.background_sound_id as BackgroundSoundId)) throw new TypeError("persona background sound is not supported");
}
...
export function personaParametersOfSettings(settings: PersonaSettings): PersonaParameterValues {
  const controls = validPersonaControls({ ...settings, executionPolicyVersion: PERSONA_EXECUTION_POLICY_VERSION }, settings.models.mode);
  return {
    ...
    ...(settings.models.mode === "separate" ? { interruption_level: controls.interruptionLevel } : {}),
  };
}
...
export function personaModelsOfParameters(values: PersonaParameterValues): PersonaModels {
  ...
  return structuralPersonaModels({
    ...
    tts: { provider: values.tts_provider, model: values.tts_model, voiceId: values.tts_voice_id, speed: Number(values.tts_speed ?? 1) },
  });
}

export function personaControlsOfParameters(values: PersonaParameterValues): PersonaControls {
  const mode = values.speech_mode === "live" ? "live" : "separate";
  return validPersonaControls({
    language: values.language,
    executionPolicyVersion: values.execution_policy_version ?? PERSONA_EXECUTION_POLICY_VERSION,
    backgroundSoundId: values.background_sound_id ?? "none",
    ...(mode === "separate" ? { interruptionLevel: values.interruption_level === "off" || values.interruption_level === undefined ? "none" : values.interruption_level } : {}),
  }, mode);
}
...
export function currentPersonaParameterDefaults(
  ...
  const currentContract = personaParameterContract(models, currentControls);
  return {
    contract: currentContract,
    values: personaParametersOfSettings({ models, ...currentControls }),
  };
}
...
export function ticket01PersonaParameterContract(
  ...
  controls: Omit<HistoricalPersonaControls, "backgroundSoundId" | "backgroundVolume" | "interruptionLevel" | "speechSpeed"> = { language: "en-US", emotion: "neutral", accent: "voice_default", speechVolume: 1, executionPolicyVersion: 1 },
...
export function ticket02PersonaParameterContract(
  models: PersonaModels = RECOMMENDED_PERSONA_MODELS,
  controls: Omit<HistoricalPersonaControls, "interruptionLevel" | "speechSpeed"> = { language: "en-US", emotion: "neutral", accent: "voice_default", speechVolume: 1, executionPolicyVersion: 1, backgroundSoundId: "none", backgroundVolume: BACKGROUND_VOLUME_DEFAULT },
): readonly GraderParameter[] {
  const checked = controls;
  return [
...
export function personaParameterContract(
  ...
  const selectedControls = controls ?? { language: "en-US", executionPolicyVersion: PERSONA_EXECUTION_POLICY_VERSION, backgroundSoundId: "none", ...(currentModels.mode === "separate" ? { interruptionLevel: "none" as const } : {}) };
  const checked = validPersonaControls({ ...selectedControls, executionPolicyVersion: PERSONA_EXECUTION_POLICY_VERSION }, currentModels.mode);
  const modelFields = modelParameterValues(currentModels);
  ...
  return [
    ...
    { key: "background_sound_id", label: "Background sound", valueType: "string", defaultValue: checked.backgroundSoundId, unit: null, minimum: null, maximum: null },
    ...(currentModels.mode === "separate" ? [{ key: "interruption_level", label: "Interruption level", valueType: "string" as const, defaultValue: checked.interruptionLevel!, unit: null, minimum: null, maximum: null }] : []),
  ];
}
...
export function preCategoricalPersonaParameterContract(
  models: PersonaModels = RECOMMENDED_PERSONA_MODELS,
  controls: Omit<HistoricalPersonaControls, "speechSpeed" | "interruptionLevel"> & { readonly interruptionLevel: "off" | "occasional" | "frequent" } = { language: "en-US", emotion: "neutral", accent: "voice_default", speechVolume: 1, executionPolicyVersion: 1, backgroundSoundId: "none", backgroundVolume: BACKGROUND_VOLUME_DEFAULT, interruptionLevel: "off" },
): readonly GraderParameter[] {
  return [
...
export function validatePersonaParameterContract(value: unknown): readonly GraderParameter[] {
  ...
  if (isTicket01) validateHistoricalControls(defaults);
  if (isTicket02) validateHistoricalControls(defaults, true);
...
export function validatePersonaParameterValues(contract: unknown, values: unknown): PersonaParameterValues {
  ...
  if (checked.speech_mode === "separate" || checked.speech_mode === "live") { personaModelsOfParameters(checked); personaControlsOfParameters(checked); }
  else if (checkedContract.some((field) => field.key === "speech_speed")) { personaModelsOfParameters(checked); personaControlsOfParameters(checked); }
  ...
  } else if (checkedContract.some((field) => field.key === "background_sound_id")) validateHistoricalControls(checked, true);
  else if (checkedContract.some((field) => field.key === "language")) validateHistoricalControls(checked);
  return checked;
}
