...
BEGIN
  ...
  ELSIF NOT keys @> ARRAY['language'] AND (parameters->>'tts_speed')::numeric NOT BETWEEN 0.6 AND 1.5 THEN RETURN false;
  END IF;
  IF keys @> ARRAY['background_sound_id'] AND (
    jsonb_typeof(parameters->'background_sound_id') IS DISTINCT FROM 'string'
  ...
  IF keys @> ARRAY['speech_speed'] THEN
    IF jsonb_typeof(parameters->'speech_speed') IS DISTINCT FROM 'string'
      OR parameters->>'speech_speed' NOT IN ('slow','normal','fast')
      OR jsonb_typeof(parameters->'interruption_level') IS DISTINCT FROM 'string'
    ...
    THEN RETURN false; END IF;
  ELSIF keys @> ARRAY['interruption_level'] AND (
    jsonb_typeof(parameters->'interruption_level') IS DISTINCT FROM 'string'
    OR parameters->>'interruption_level' NOT IN ('off','occasional','frequent')
  ) THEN RETURN false; END IF;
  RETURN true;
END;
$$;
