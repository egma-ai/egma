...
function record(
  spanId: string,
  changes: Partial<NewUsageRecord> = {},
): NewUsageRecord {
  return {
    ...
    rawUsage: { prompt_tokens: 1400, completion_tokens: 100 },
    ...changes,
  };
}
beforeAll(async () => {
...
it("prices all three quantities once and preserves their separate rates through replay", async () => {
  ...
  expect(Object.keys(priced[0]?.usage?.price?.pricedBy ?? {})).toEqual([
    "input_tokens",
    "cached_input_tokens",
    "output_tokens",
  ]);
  expect(
    new Set(Object.values(priced[0]?.usage?.price?.pricedBy ?? {})).size,
  ).toBe(3);
  ...
  expect(
    await readPlatformUsageTotal({
      ...
      occurredAtOrAfter: floor,
    }),
  ).toEqual({ amountMicros: 240n, requests: 1n });
  expect(
    await readOrganizationUsage(auth, {
      from: floor,
...
it("uses the complete tenant/project/trace/span identity and counts customer-funded usage only in the read surface", async () => {
  ...
  for (const context of [one, two, other])
    await appendSpans(context, await priceUsageSpans(context, [source]));
  await appendSpans(
    one,
    await priceUsageSpans(one, [
      providerUsageSpan(
        record("0000000000000003", { paymentSource: "customer" }),
      ),
    ]),
  );
  expect(
    await readPlatformUsageTotal({ organizationId, occurredAtOrAfter: floor }),
  ).toEqual({ amountMicros: 480n, requests: 2n });
  expect(
    await readPlatformUsageTotal({
  ...
  expect(
    await readOrganizationUsage(one, {
      from: floor,
      to: new Date("2026-09-09"),
    }),
  ).toMatchObject({ amountMicros: 720, requests: 3 });
});
...
it("includes delayed visible usage in the next snapshot and excludes old usage even when it arrives late", async () => {
  ...
  const pending = await priceUsageSpans(context, [
    providerUsageSpan(record("0000000000000004")),
  ]);
  expect(
    await readPlatformUsageTotal({
  ...
  await appendSpans(
    context,
    await priceUsageSpans(context, [
      providerUsageSpan(
        record("0000000000000005", { occurredAt: new Date("2026-09-07") }),
      ),
    ]),
  );
  expect(
...
it("keeps first receipt and one price when identical acceptance races and later replay crosses an interval", async () => {
  ...
  const first = providerUsageSpan(
    measurement,
    new Date("2026-09-08T12:01:00Z"),
  );
  const second = providerUsageSpan(
    measurement,
    new Date("2026-09-08T12:01:01Z"),
  );
  const [left, right] = await Promise.all([
    priceUsageSpans(context, [first]),
    priceUsageSpans(context, [second]),
  ]);
  await Promise.all([appendSpans(context, left), appendSpans(context, right)]);
  await store.command("OPTIMIZE TABLE spans FINAL");
  const replay = await priceUsageSpans(context, [
    providerUsageSpan(measurement, new Date("2026-09-28")),
  ]);
...
it("cannot replace a priced usage span with non-usage evidence even after storage merges", async () => {
  const context = { ...auth, organizationId: newId("org") };
  const [priced] = await priceUsageSpans(context, [
    providerUsageSpan(record("0000000000000007")),
  ]);
  ...
  await appendSpans(context, [
    { ...span, kind: "conversation", name: "conversation" },
  ]);
  await store.command("OPTIMIZE TABLE spans FINAL");
  expect(
    await readPlatformUsageTotal({
...
it("retains conflicting price evidence across merges and refuses to choose a charge", async () => {
  const context = { ...auth, organizationId: newId("org") };
  const [priced] = await priceUsageSpans(context, [
    providerUsageSpan(record("0000000000000008")),
  ]);
  ...
  const conflicting = {
    ...
    usage: {
      ...priced.usage,
      price: { ...priced.usage.price, amountMicros: 999 },
    },
  };
  await expect(priceUsageSpans(context, [conflicting])).rejects.toThrow(
  ...
  ).rejects.toThrow("conflicting immutable identities");
});

it("counts one identity when ClickHouse commits but its success response is lost", async () => {
  const context = { ...auth, organizationId: newId("org") };
  const priced = await priceUsageSpans(context, [
    providerUsageSpan(record("0000000000000009")),
  ]);
  await store.command(
    "ALTER TABLE spans MODIFY SETTING non_replicated_deduplication_window = 0",
  );
  ...
  const proxy = createServer((incoming, response) => {
    const upstreamUrl = new URL(incoming.url ?? "/", store.url);
    const upstream = request(
      ...
      (reply) => {
        reply.resume();
        reply.on("end", () => {
          // Only after real ClickHouse has accepted the complete INSERT do we
          // break the downstream connection. The writer cannot know it committed.
          committedResponseLost = reply.statusCode === 200;
          response.destroy();
        });
      },
    );
    upstream.on("error", (cause) => response.destroy(cause));
  ...
  const throughProxy = new URL(store.url);
  throughProxy.port = String(address.port);
  await disconnectClickHouse();
  connectClickHouse({ clickhouseUrl: throughProxy.toString() });
  try {
    await expect(appendSpans(context, priced)).rejects.toThrow();
    expect(committedResponseLost).toBe(true);
  } finally {
    await disconnectClickHouse();
