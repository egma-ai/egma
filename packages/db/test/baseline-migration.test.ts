...
describe("the fresh Postgres baseline", () => {
  it("installs in one file and keeps organization data on repeated boot", async () => {
    ...
    expect((await store.sql("select name from egma_meta.migration")).rows).toEqual([
      { name: BASELINE },
    ]);
  });
  ...
  it("applies once when API instances boot concurrently", async () => {
    ...
    expect(results.flatMap((result) => result.applied)).toEqual([BASELINE]);
    expect(
  ...
  it("rolls back failed installation before recording the baseline and retries cleanly", async () => {
    const sql = await readFile(path.join(MIGRATIONS_DIRECTORY, BASELINE), "utf8");
    await writeFile(
    ...
    expect(
      (await store.sql("select tablename from pg_tables where schemaname = 'public'")).rows,
    ).toEqual([]);
    expect((await store.sql("select name from egma_meta.migration")).rows).toEqual([]);
    await writeFile(path.join(directory, BASELINE), sql);
    expect((await runMigrations(database.url, directory)).applied).toEqual([BASELINE]);
  });
  ...
  it("refuses unknown recorded history even when the baseline matches", async () => {
    ...
    expect(
      (await store.sql("select name from egma_meta.migration order by name")).rows,
    ).toEqual([{ name: BASELINE }, { name: "9999_unknown.sql" }]);
  });
  ...
  it("installs validated constraints and enabled custom execution guards", async () => {
    ...
    expect(
      (await store.sql(`select conname from pg_constraint
        where connamespace = 'public'::regnamespace and not convalidated`)).rows,
    ).toEqual([]);
    expect(
      (await store.sql(`select tgname from pg_trigger
        where not tgisinternal and tgenabled <> 'O'`)).rows,
    ).toEqual([]);
    const { rows } = await store.sql<{ tgname: string }>(
      "select tgname from pg_trigger where not tgisinternal",
    );
    expect(rows.map((row) => row.tgname)).toEqual(expect.arrayContaining([
      "run_lifecycle_guard",
