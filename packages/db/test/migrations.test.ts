...
describe("the persona catalog ownership cutover (0045)", () => {
  ...
  it("keeps reasoning policy out of the stored persona shape", async () => {
    ...
    ).rejects.toThrow(/persona_version_models_valid/u);
  });

  it("runs once and restores immutable persona versions", async () => {
    const again = await runMigrations(database.url, before);
