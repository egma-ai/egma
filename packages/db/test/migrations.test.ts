...
const REMOVED_AUDIO_ROLLBACK_MIGRATION = `ALTER TABLE "simulation" DROP CONSTRAINT "simulation_audio_band_is_a_rate";--> statement-breakpoint
...
describe("the simulation audio rollback column after an earlier candidate removed it", () => {
  let database: EmptyDatabase;
  let client: pg.Client;
  let beforeRepair: string;

  beforeAll(async () => {
    database = await createEmptyDatabase("restore_audio_rollback_column");
    beforeRepair = await mkdtemp(
      path.join(os.tmpdir(), "egma-before-audio-rollback-repair-"),
    );

    for (const migration of await readMigrations()) {
      if (migration.name < "0034") {
        await writeFile(
          path.join(beforeRepair, migration.name),
          migration.sql,
        );
      }
    }
    await writeFile(
      path.join(beforeRepair, "0034_cute_nico_minoru.sql"),
      REMOVED_AUDIO_ROLLBACK_MIGRATION,
    );
    await runMigrations(database.url, beforeRepair);
  ...
  afterAll(async () => {
    await client.end();
    await rm(beforeRepair, { recursive: true, force: true });
    await database.drop();
  });

  it("repairs the exact candidate state before a current run insert", async () => {
    const before = await client.query<{ conname: string }>(
      `select conname from pg_constraint
        where conrelid = 'simulation'::regclass
          and conname in (
    ...
    expect(before.rows.map(({ conname }) => conname)).toEqual([
      "simulation_audio_facts_are_voice_facts",
      "simulation_report_only_when_ended",
    ]);

    const currentWatch = (await readMigrations()).find((migration) =>
      migration.name.startsWith("0034_"),
    );
    const repair = (await readMigrations()).find((migration) =>
      migration.name.startsWith("0035_"),
    );
    if (currentWatch === undefined) throw new Error("current 0034 is missing");
    if (repair === undefined) throw new Error("0035 is missing");
    await writeFile(
      path.join(beforeRepair, currentWatch.name),
      currentWatch.sql,
    );
    await writeFile(path.join(beforeRepair, repair.name), repair.sql);

    const result = await runMigrations(database.url, beforeRepair);
    expect(result.applied).toEqual([currentWatch.name, repair.name]);
    ...
    const constraints = await client.query<{
      ...
      `select conname, pg_get_constraintdef(oid) as definition, oid
         from pg_constraint
        where conrelid = 'simulation'::regclass
          and conname in (
            'simulation_report_only_when_ended',
    ...
    expect(
      ...
      {
        conname: "simulation_audio_band_is_a_rate",
        definition:
          "CHECK (((measured_audio_band_hertz IS NULL) OR (measured_audio_band_hertz > 0)))",
      },
      {
        conname: "simulation_audio_facts_are_voice_facts",
        definition:
          "CHECK (((modality = 'voice'::text) OR ((measured_audio_band_hertz IS NULL) AND (recording_reference IS NULL))))",
      },
      {
        conname: "simulation_report_only_when_ended",
        definition:
          "CHECK (((ended_at IS NOT NULL) OR ((recording_reference IS NULL) AND (measured_audio_band_hertz IS NULL))))",
      },
    ]);

    const current = drizzle(client, { schema, casing: "snake_case" });
    const insert = current.insert(schema.simulation).values({
      id: newId("sim"),
      runId: newId("run"),
    ...
    expect(insert.toSQL().sql).toContain('"measured_audio_band_hertz"');
    ...
    const beforeNoOp = constraints.rows.map(({ conname, oid }) => ({
      conname,
      oid,
    }));
    await client.query(repair.sql);
    const afterNoOp = await client.query<{ conname: string; oid: number }>(
    ...
    expect(afterNoOp.rows).toEqual(beforeNoOp);
  });
});
