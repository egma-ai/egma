...
export const replayMonitoringImportFailure = <ThrowOnError extends boolean = false>(parameters: {
    failureId: string;
    projectId?: string;
}, options?: Options<never, ThrowOnError>): RequestResult<ReplayMonitoringImportFailureResponses, ReplayMonitoringImportFailureErrors, ThrowOnError> => {
    const params = buildClientParams([parameters], [{ args: [{ in: 'path', key: 'failureId' }, { in: 'query', key: 'projectId' }] }]);
    return (options?.client ?? client).post<ReplayMonitoringImportFailureResponses, ReplayMonitoringImportFailureErrors, ThrowOnError>({
        security: [{ scheme: 'bearer', type: 'http' }, {
                in: 'cookie',
                name: 'egma.session_token',
                type: 'apiKey'
