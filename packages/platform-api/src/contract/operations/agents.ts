...
const connectionInput = {
  ...
  properties: {
     ...
     * `agents:discover` listed it.
     *
     * Required for every Retell connection. Egma confirms it against Retell
     * with the key in `credentials`, or with the key already sealed on the
     * agent, immediately before the connection is written — so a number that
     * has stopped answering for that agent is refused rather than stored.
     */
    platformAgentId: { type: "string" },
    /**
