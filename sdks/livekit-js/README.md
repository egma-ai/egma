...
It also records committed assistant messages, including greetings made with `session.say()`, when LiveKit does not include them in a native turn trace. Repeated phrases remain separate messages. Interrupted messages retain LiveKit's interruption flag and committed text; that text can include words that did not finish playing. Speech with `addToChatCtx: false` is not a committed conversation message.
...
It sends production traces and committed assistant messages to egma Monitoring. `monitor(ctx)` also works when you only need the native traces. The helper does nothing in simulation rooms.
