...
```bash
pip install --upgrade "egma[livekit]"
```

For a repo using uv:

```bash
uv add --upgrade "egma[livekit]"
```

The SDK supports Python 3.11 or newer and `livekit-agents>=1.6.6,<1.9`, including LiveKit 1.8. The `livekit` extra holds that range and OpenAI Python 2. Check compatibility with the worker's existing dependencies before upgrading and keep the resolved versions in the repo's lockfile.
