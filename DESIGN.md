...
- Do not recolor, stretch, rotate, outline, shadow, or animate the logo. One exception, scoped and approved on the Paper page `12 — Run screens · Polish v2`: while a selected simulation is queued or running, the run detail's Results tab shows the mark centred at 56px in place of the summary row (the Transcript tab keeps streaming), breathing in opacity on the status-pulse token and solid under reduced motion. Nothing else moves the mark, and nothing else may. (Developer decision, 2026-09-07.)
