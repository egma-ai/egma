...
- A page below a section carries the trail into its record in that bar, and **the trail is one line that ends with the record** — `Tests / Livekit agent suite`. Every step is in the trail's own 14px type, the parent steps are links, and the last step is the page's own heading. The bar used to draw the trail short of the record and the record beside it as a larger heading, which read as a small underlined link stuck to a big title with no separator between them. (Developer decision, 2026-08-26. This replaces the never-repeat-the-title rule read off the boards on 2026-08-23. A page with no trail still draws its title alone in the bar.)
