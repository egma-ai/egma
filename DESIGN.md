...
- **One label grammar, everywhere.** A mandatory field's label ends in `*`, printed in the label's own ink right after the last word — `Name*`, `Test suite*`. An optional field's label carries no suffix at all; `[optional]` is retired. A field carries at most one faint help line, and that line says what to write or where the value comes from — never how Egma stores it. Text inputs are 44px on white with 14px text and a 14px faint placeholder; every dropdown is 44px on the soft grey surface with 14px text, so a field that opens a list looks different from one that takes typing. The persona pages, which set this look on 2026-09-15, keep their placeholders as writing guidance and carry no help lines. (Developer decisions, 2026-08-24; the persona look taken product-wide from Paper page `14 — Create a run · Persona controls`, variant C, 2026-09-15 evening.)
