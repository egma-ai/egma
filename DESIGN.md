...
The product's shape changed on 2026-08-23 with that approval, from the Paper file "Egma — Design system and product UI". Five rules moved, and each is marked where it is written: **one radius, and it is 0px**; Egma identity returned to the signed-in sidebar; the **primary action is the wash button**, not a Deep Ember block; **`system-ui` leads the font stack**; and the **side sheet** is where one record is created, read and edited. On 2026-08-24, the approved Paper refinement replaced the full sidebar wordmark with the Egma mark beside organization context and made the account avatar square. The same session's agents-and-tests rework added three rules and changed one: the `*` and `[optional]` label grammar, the Ember top-line on a segmented control's chosen segment, square status markers, and — the one change — **quiet text-field focus**. On 2026-08-26, the run-flow refinement removed decorative markers from settled run states, reserved a spinner for active work, and moved grader verdict meaning into explicit `Result · Passed` or `Result · Failed` text. On 2026-09-07, the run-review polish gave run and simulation status a filled square that pulses while work is unsettled, retired the run spinner, moved the run facts off the run detail page onto the runs table, put Regrade in the tab rail, and made grader sections collapsible; the same evening's review on Paper page `12` took the sublines and search off the run screens, moved Regrade onto the Graders line as an icon, put notices under the summary row, and let the Egma mark breathe as the one loading state. On 2026-09-15, the persona pages were rebuilt from Paper page `12 — Personas · Complete flow + Dropdowns`: three caps groups with hairlines only between them, no collapsing sections, `Text-to-speech` / `Speech-to-text` / `LLM` / `Realtime LLM` as the subsection names, a solid Deep Ember primary on those pages alone, `Clone` back in the read page's title bar, and a hairline over the sidebar's account control. The palette and the logo's own treatment rules did not move.
...
- Do not recolor, stretch, rotate, outline, shadow, or animate the logo. One exception, scoped and approved on the Paper page `12 — Run screens · Polish v2`: while a selected simulation is queued or running, the run detail shows the mark centred at 56px, breathing in opacity on the status-pulse token and solid under reduced motion. The Results tab shows it in place of the summary row. The Transcript tab shows it in place of the recording and the transcript until the first turn or tool call arrives, and streams from then on: a simulation that has sent nothing yet has no evidence to draw, and two empty blocks read as failed work. Nothing else moves the mark, and nothing else may. (Developer decision, 2026-09-07; the Transcript tab added by developer decision, 2026-09-15.)
...
- **Run and simulation status is a filled square before its word.** A run's status word and a simulation's status word each carry a 10px square filled with the state colour: yellow for `Pending`, `Running`, `Queued` and `Grading`, green for `Completed` and for a simulation whose every grader passed its own threshold, red for `Execution failed`, `Grading failed` and a simulation where any grader failed or errored, and grey for `Canceled` and for `Not graded`, when no grader was selected. The yellow square pulses in opacity while the work is unsettled and stays solid under reduced motion; the spinner is retired. A graded simulation's word is the count, `2/3 passed`, never `1 failed` in a list. There is no overall threshold: ADR-0017 stands, and the square reads the individual grader results. `claimed` is shown as `Queued`. The square lives on the runs table row and the simulation list row; the selected simulation's heading on the run detail carries the name alone, because the list beside it already says the state. (Developer decision, 2026-09-07 evening.) On Run Results surfaces, grader verdicts still use `Result · Passed` or `Result · Failed`, with colour only on the verdict word and no marker. (Developer decision, 2026-09-07. This replaces the 2026-08-26 text-first rule for run status; the same session made canceled grey rather than warning-coloured on both surfaces.)
...
- The account control stays at the bottom, in a 56px band — the organization bar's own height — under a hairline that spans the sidebar. The control draws no box of its own, at rest or under a pointer — a pointer only tints its fill; only the avatar square is boxed. (Read off Paper page `12`, 2026-09-15; the band by developer decision, 2026-09-15 evening.)
...
- A page that owns the viewport — settings, the persona form, the run workbench on desktop — pins the document: its body scrolls inside the shell, and the document itself cannot be scrolled, so nothing appended after the shell by a browser extension or a development overlay can pull the page down. (Developer decision, 2026-09-15 evening.)
...
- A run detail is a bounded workbench on desktop. The simulation list, selected-simulation heading and tab rail stay in place; only the active Results or Transcript panel scrolls. At 900px and below it returns to normal document flow. The run facts — status, start, suite, agent, connection — left the detail page on 2026-09-07 because the runs table already says them; the trail names the run and `Cancel run` stays in the title bar. (Developer decision, 2026-08-26, from the run-detail results review; run facts removed by developer decision, 2026-09-07.) The simulation list head reads `Simulations · n`, the count a faint annotation after the word, and carries no search field. The list head and the selected-simulation heading are both 56px tall so their hairlines meet at the divider; the heading is the test name at the 16px body step, weight 500, with no square, no status word and no action. While the selected simulation is queued or running, the Results tab shows the loader described under **Brand**; the Transcript tab shows the same loader until its first turn or tool call arrives, then streams. (Developer decision, 2026-09-07 evening, Paper page `12`; the Transcript tab added by developer decision, 2026-09-15.)
...
| Toolbar row | 52px |
| Persona form | 720px |
| Persona read page | 960px |
| Persona read label lane | 240px |
...
- Primary: Ember Wash fill, Deep Ember text, a one-pixel neutral hairline, no corner. 36px in a toolbar, 44px in a form. (Developer decision, 2026-08-23. The Deep Ember block with white text is retired; no variant draws it.) **The persona pages are the one exception:** their `Next` and `Create persona` are a solid Deep Ember block with paper text, 44px tall with 16px side padding, and the `Cancel` beside it is text alone with 12px side padding, 8px apart. Every other page keeps the wash. (Developer decision, 2026-09-15, from Paper page `12`.)
...
- A section inside a page never out-sizes the page: a section title is the 16px weight-500 step the title bar and the empty-state card use, and the 32px above it is what sets it apart. A form's submit is the height of the fields it finishes — 44px — and a form that stands over a table takes the table's width, so the two panels share both edges. (Developer decision, 2026-09-15 evening, from the API keys review.)
- Group related fields on Pure Paper surfaces.
- Keep labels visible. Placeholder text is not a label.
- **One label grammar, everywhere.** A mandatory field's label ends in `*`, printed in the label's own ink right after the last word — `Name*`, `Test suite*`. An optional field's label carries no suffix at all; `[optional]` is retired. A field carries at most one faint help line, and that line says what to write or where the value comes from — never how Egma stores it. Text inputs are 44px on white with 14px text and a 14px faint placeholder; every dropdown is 44px on the soft grey surface with 14px text, so a field that opens a list looks different from one that takes typing. On a touch screen every control keeps the 16px step, because iOS zooms the page into a focused control set smaller than that. The persona pages, which set this look on 2026-09-15, keep their placeholders as writing guidance and carry no help lines. (Developer decisions, 2026-08-24; the persona look taken product-wide from Paper page `14 — Create a run · Persona controls`, variant C, 2026-09-15 evening.)
...
- **An annotation is a quiet fact beside a label, never a second name for the field.** A field may carry one short annotation after its label — `scores · 1` — in the same faint ink as a help line, on the interpunct grammar this file already sets for `Result · Passed`. It says what the value means rather than what to write, so it sits outside the `<label>` element and never joins the name a screen reader announces; the control points at it with `aria-describedby` instead. Square brackets belong to no label any more, so an annotation never wears them. A field carries an annotation or a help line, not both. (Developer decision, 2026-08-29.)
...
- A row's standing is the compact count chip on a transparent ground: one hairline, one word in sentence case, and a tone that supports the word. A value that names a role is written the way a person says it — `Admin`, not the contract's `admin` — while the lowercase string stays on what the control sends. Settings → People follows both, with its row actions in the shared row menu. (Developer decision, 2026-09-15 evening.)
...
- A list's date column is the absolute short date — `Aug 16, 2026` — in tabular numerals, with the exact instant kept on the element. A relative age belongs in a sentence ("started just now", "last received 2 min ago") and never in a column: a column of ages cannot be scanned, and two rows a minute apart read the same for the whole of the first hour. One column names a moment rather than a day, and it keeps its precision in the same shape: the runs table's `Started` shows the date over the clock, `Sep 7, 2026` then `18:48`, since 2026-09-07. (Read off `6ZJ-0`, `8TQ-0` and `8P4-0`, 2026-08-23; runs minute by developer decision, 2026-09-07.)
- The runs table names the run, its test suite, agent, connection, start and status. The agent and connection columns show the name alone, with no modality or product-label line under it. `Started` shows the short date on one line and the 24-hour clock under it in faint ink, so six columns fit the page frame with no horizontal scroll. (Developer decision, 2026-09-07; sublines removed and the clock stacked by developer decision, 2026-09-07 evening.)

### Run evidence

- Results begin with one compact row above every grader: Graders passed (`2/3`, with `· 1 errored` added only when a grader errored; a failed grader adds no word, because the fraction already says it — developer decision, 2026-09-16), Duration, Total turns and P90 turn latency. Keep all four facts on one desktop row, stacking each fact's label over its value when the detail pane is narrow. The values are set in the sans face on tabular figures, the same face as their labels; the mono face stays with identifiers and transcript timestamps. An execution-failure notice sits above this row, where it cannot be missed. Grading-in-progress, grading-failed, and canceled notices sit below the row and above the graders. (Developer decision, 2026-09-08.) The latency is the platform's recorded `turn_response_latency.p90`, shown in milliseconds with at most three significant digits; mark it as partial when the trace was truncated. Do not repeat the full latency list or the frozen grading plan on this surface; each grader section already carries its version and pass threshold. (Developer decision, 2026-08-26; metric label renamed by developer decision, 2026-08-27; Total avg score replaced by Graders passed, developer decision 2026-09-07.)
- Grader sections are collapsible. A `Graders` line carries no count, because the summary row already says it; right after the word sits the Regrade control, a 22px hairline square with a 13px refresh icon, named `Regrade this simulation` for assistive technology, grown to the 44px target on coarse pointers, dimmed while nothing can be regraded, and opening the same confirmation as before. Each section's header row shows the name, `Result · Passed`, `Result · Failed` or `Result · Error`, and `Score x · Threshold y`. Opening a section shows only its evidence: the expected-behavior table, or the grader's finding when it has no table, or the error when it errored. No version line. Failed, errored and still-grading sections open by default; passed sections start collapsed. (Developer decision, 2026-09-07; count, version line and finding removed and Regrade moved by developer decision, 2026-09-07 evening.)
- Regrade lives on the `Graders` line, described above, and applies to the selected simulation. It left the heading row and the tab rail on 2026-09-07 evening.
...
- Production and simulation transcripts use the same compact continuous rail: one outer hairline, no gaps between turns, and one hairline between events. Time and speaker share a fixed 104px metadata lane; spoken rows are at least 40px and nested tool rows are at least 36px, while controls still grow to the 44px pointer target on coarse pointers. An Agent-owned tool stays nested inside that Agent turn in event order; an orphan remains visible at the top level. A successful tool uses its status word without an empty square. A failed or errored tool keeps the status word and adds the filled Failure square. (Developer decision, 2026-08-28, from Paper compact transcript Version 3.)
...
- **Personas are the scoped exception.** A persona's architecture is chosen on a setup page, then create, read, and clone each use a full page with the production shell and breadcrumb. Saved custom and predefined personas are read-only; changes start an editable clone draft whose architecture cannot change. The list stays a separate page. Every persona page is three groups — `METADATA`, `WHO THEY ARE`, `SETTINGS` — each headed by a 16px weight-500 caps label in ink, with a hairline only between groups: 16px under the line on the form, 20px on the read page, and none above the first group. Inside Settings, subsections are plain headers with no line between them and no toggle: nothing on a persona page collapses. On the form a subsection header is the 16px step, and `Language*` is both the header and the label of its one dropdown; on the read page a subsection header is the 14px step in ink over 14px faint row labels, each label in a fixed 240px lane with its value beside it. The subsections are `Language`, `Text-to-speech`, `Speech-to-text`, `LLM` and `Advanced Settings`, or `Realtime LLM` in place of the middle three, whose fields are `Provider`, `Model`, `Voice` and `Reasoning LLM`, one dropdown of the supported reasoning models. Persona pages carry no tips block. The read page carries `Clone` at the right of its title bar as the standard 36px wash button with the copy icon, and the persona table menu keeps its own. The setup step is two white cards under a 16px title, each with only a 2px top edge — Ember on the chosen card — a 16px card title (`Cascaded Architecture`, `Realtime voice`), a 14px faint description, the shared radio at the right, and grey 14px chips naming the models in the pipeline. Agents, connections, and tests continue to use the side-sheet rule above. (Developer decisions, 2026-09-14 from Paper page `12 — Personas · Complete flow + Dropdowns`, revised on that page 2026-09-15; `Advanced Settings`, the card title, the 14px card copy and the standard Clone button by developer decision, 2026-09-15 evening.)
...
- Side-sheet footers place secondary actions on the left and the primary action on the right. (Developer decision, 2026-09-08.)
...
- Menus and popovers scale from the trigger origin. A dropdown opens below its trigger when the window has room there and above it when it does not, and its list is capped to that room and scrolls inside it. Opening a dropdown never moves the page: no spacer under the field, no scrolling the trigger into view. (Developer decision, 2026-09-15 evening; this replaces the always-below persona exception of the same day.)
...
| Status pulse | 1120ms | The filled status square, and the loader's Egma mark, breathing while work is unsettled; four drawer enters |
...
| Status square, loader mark | Show unsettled work | Opacity pulse on `--ease-in-out`; solid under reduced motion |
