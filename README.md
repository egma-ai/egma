...
The current catalog offers OpenAI `gpt-4o-mini` and `gpt-5.6-terra` for the
LLM; Deepgram `nova-3-general` and OpenAI `gpt-live-transcribe` for STT; and
Cartesia `sonic-3.5` and OpenAI `gpt-4o-mini-tts` for TTS. Reasoning effort is
an LLM catalog capability. The TTS selection also owns the voice id and speed.
The platform Default Persona v1 selects `gpt-5.6-terra` with reasoning effort
`none`. Persona traits contain behavior only.
